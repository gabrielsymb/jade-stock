...
- endpoint `POST /v1/kanban/politicas`;
...
- endpoint `POST /v1/curva-abcd/processar`;
...
- persistência em `politica_reposicao`;
- endpoint `POST /v1/giro/processar`;
- testes unitários + API (`inmemory` e `postgres`) + integração PostgreSQL de caso de uso.
...
- endpoint `POST /v1/sazonalidade/processar`;
- eventos:
...
- endpoint `POST /v1/orcamento/simular`;
- testes unitários + API (`inmemory` e `postgres`) + integração PostgreSQL de caso de uso.
