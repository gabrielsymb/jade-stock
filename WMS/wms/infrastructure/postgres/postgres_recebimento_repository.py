...
class PostgresRecebimentoRepository:
    ...
    def salvar_recebimento(self, payload: dict) -> str:
        ...
        with self.connection.cursor() as cursor:
            ...
            for item in itens:
                sku_ref = item.get("sku_id") or item.get("sku_codigo")
                sku_id = self._resolve_sku_id(cursor, sku_ref)
                if not sku_id:
                    raise RuntimeError(f"SKU nao encontrado: {sku_ref}")
    ...
    def _resolve_sku_id(self, cursor: Any, sku_ref: str | None) -> str | None:
        if not sku_ref:
            return None
        sql = """
            SELECT sku_id
