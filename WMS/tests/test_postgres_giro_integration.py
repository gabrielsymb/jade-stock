"""Testes de integracao PostgreSQL para Giro operacional (extended)."""
...
class PostgresGiroIntegrationTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.schema_name = f"wms_it_giro_{uuid4().hex[:8]}"
    ...
    def setUp(self) -> None:
        with self.connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                VALUES ('itm_001', 'Item Giro')
                """
            )
            cursor.execute(
                """
                INSERT INTO sku (sku_id, sku_codigo, sku_nome, item_master_id, status_ativo)
                VALUES ('sku_001', 'sku_001', 'SKU Giro', 'itm_001', TRUE)
                """
            )

    def test_processar_giro_persiste_politica(self) -> None:
        ...
        with postgres_transaction(self.connection):
            out = use_case.execute(
                ProcessarGiroEstoqueInput(
                    operador="op_giro",
                    correlation_id="corr_giro_pg_001",
                    itens=[
                        ItemGiroEstoqueInput(
                            sku_id="sku_001",
                            classe_abc="A",
