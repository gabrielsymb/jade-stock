...
class ProcessarGovernancaOrcamentaria:
    ...
    def execute(
        ...
        consumo_total_projetado = data.consumo_atual_total + data.valor_compra_sugerida
        consumo_categoria_projetado = (
            data.consumo_atual_categoria + data.valor_compra_sugerida
        )

        if consumo_categoria_projetado > limite_categoria:
            alertas.append("compra_acima_orcamento_categoria")
        if consumo_total_projetado > limite_total:
            alertas.append("compra_acima_orcamento_total")
        if (
            consumo_categoria_projetado > limite_categoria
            and consumo_total_projetado <= limite_total
        ):
